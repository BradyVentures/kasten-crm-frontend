...
function EditTodoModal({
  ...
  const [form, setForm] = useState({
    ...
    customer_service_id: todo.customer_service_id || '',
  });
  const [loading, setLoading] = useState(false);
  const [users, setUsers] = useState<User[]>([]);
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [customerServices, setCustomerServices] = useState<{ id: string; service_name: string }[]>([]);
  ...
  // Load services when customer changes
  useEffect(() => {
    if (!form.customer_id) { setCustomerServices([]); return; }
    api.get(`/customers/${form.customer_id}`).then((r) => {
      setCustomerServices(r.data.services || []);
    }).catch(() => setCustomerServices([]));
  }, [form.customer_id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.put(`/todos/${todo.id}`, form);
      onSaved();
      onClose();
    } catch {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Todo bearbeiten">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm text-bd-text-secondary mb-1">Titel *</label>
          <input
        ...
        {form.customer_id && customerServices.length > 0 && (
          <div>
            <label className="block text-sm text-bd-text-secondary mb-1">Service (optional)</label>
            <select
              value={form.customer_service_id}
        ...
        <p className="text-xs text-bd-text-muted">Erstellt von: {todo.created_by_name}</p>
          ...
          >
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 text-bd-text-muted hover:text-bd-accent transition-all text-sm"
                title="Bearbeiten"
              >
                ✎
              </button>
